...
const BlogContainer = () => {
  return (
    ...
    //       <Link href={`${MEDIUM_URL}/@${post.node.author.username}/${post.node.uniqueSlug}`} target="_blank" key={key}>
    ...
    //             </h2>
    //             <h1 className="title-font text-lg font-medium text-gray-900 mb-3">{post.node.title}</h1>
    //             <p className="leading-relaxed mb-3">{post.node.virtuals.subtitle}</p>
    //           </div>
    //           <div className="flex items-center justify-between flex-wrap px-6 pb-3">
    //             <span className="text-indigo-500 inline-flex items-center md:mb-2 lg:mb-0">Learn More</span>
    //             <span className="flex space-x-2">
    //               <span>{post.node.createdAt}</span>
    //               <span>{Math.ceil(post.node.virtuals.readingTime)} min read</span>
    //             </span>
    //           </div>
    //         </div>
    //       </Link>
